...
class CSVReader extends Readable {
  constructor (options = {}) {
    ...
    this.fxaLineReader = new readline(options.fxaInputPath);
    this.salesforceLineReader = new readline(options.salesforceInputPath);
  ...
  _read () {
    if (this.fxaLine || this.salesforceLine) {
      ...
      this.counts.stats = this._calculateBucketDistribution();
