...
class CSVReader extends EventEmitter {
  ...
  run () {
    ...
    while (fxaLine || salesforceLine) {
      ...
      // than the other.
      const [ fxaUid, fxaEmail, fxaLocale, fxaCreateDate ] = this._splitLineBuffer(fxaLine, SOURCE_FXA);
      const [ salesforceUid, salesforceEmail ] = this._splitLineBuffer(salesforceLine, SOURCE_SF);
      ...
      if (normalizedFxaUid < normalizedSalesforceUid) {
        ...
        this.emit('create', {
          createDate: this._normalizeCSVDate(fxaCreateDate),
          email: fxaEmail,
          locale: fxaLocale,
          uid: fxaUid,
          timestamp
  ...
  _nextValidUid (reader, source) {
    do {
      ...
      let [ uid ] = this._splitLineBuffer(next, source);
  ...
  _splitLineBuffer(lineBuffer, source) {
    if (lineBuffer) {
      const parts = lineBuffer.toString('utf8').split(this.separator).map(item => item.trim());
      if (source === SOURCE_FXA) {
        // The email and locale in SOURCE_FXA are base64-encoded to allow simpler CSV parsing.
        parts[1] = Buffer.from(parts[1], 'base64').toString('utf8');
        parts[2] = Buffer.from(parts[2], 'base64').toString('utf8');
      }
      return parts;
    }
