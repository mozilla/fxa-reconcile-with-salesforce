...
const SalesforceUpdateEmailsReconcilingStream = require('../lib/reconcilers/salesforce-update-emails');
...
program
  .option('-e, --emails <filename>', 'Email updates CSV, format expected to be `uid,email`')
...
function usage () {
  console.log(`Usage:
  node ${programName} -e <email_updates_csv_filename> -s <salesforce_csv_filename>
`);
}

if (! program.emails || ! program.salesforce) {
  usage();
  process.exit(1);
}
...
const reconcilerStream = new SalesforceUpdateEmailsReconcilingStream({
  highWaterMark,
});
